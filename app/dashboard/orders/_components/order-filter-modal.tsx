    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-gray-800 rounded-xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto text-white">
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {paymentMethods.map((method) => (
                <label key={method} className="flex items-center space-x-2 p-2 border border-gray-600 rounded-lg hover:bg-gray-700">
                  <input
                    type="checkbox"
          ...
          {/* Delivery Methods */}
          <div>
            <label className="block text-sm font-medium mb-2 text-gray-300">Delivery Method</label>
            <div className="grid grid-cols-2 gap-2">
          ...
          {/* Total Amount Range */}
          <div>
            <label className="block text-sm font-medium mb-2 text-gray-300">Total Amount Range</label>
            <div className="grid grid-cols-2 gap-4">
              <input
                type="number"
                placeholder="Min Amount"
                value={filters.totalAmount.min}
                onChange={(e) => setFilters(prev => ({
                  ...prev,
                  totalAmount: { ...prev.totalAmount, min: e.target.value }
                }))}
                className="w-full p-2 bg-gray-700 border border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 text-white placeholder-gray-400"
              />
              <input
                type="number"
                placeholder="Max Amount"
                value={filters.totalAmount.max}
                onChange={(e) => setFilters(prev => ({
                  ...prev,
                  totalAmount: { ...prev.totalAmount, max: e.target.value }
                }))}
                className="w-full p-2 bg-gray-700 border border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 text-white placeholder-gray-400"
              />
            </div>
